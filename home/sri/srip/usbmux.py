...
class SafeStreamSocket:
	...
	def recv(self, size):
		...
		while len(msg) < size:
			...
			if chunk == '':
				raise MuxError("socket connection broken")
			msg = msg + chunk.decode('raw_unicode_escape')
		return msg
...
class BinaryProtocol(object):
	...
	def _pack(self, req, payload):
		if req == self.TYPE_CONNECT:
			return struct.pack("IH", payload['DeviceID'], payload['PortNumber']) + "\x00\x00".encode('raw_unicode_escape')
	...
	def _unpack(self, resp, payload):
		if resp == self.TYPE_RESULT:
			return {'Number':struct.unpack("I", payload.encode('raw_unicode_escape'))[0]}
		elif resp == self.TYPE_DEVICE_ADD:
			devid, usbpid, serial, pad, location = struct.unpack("IH256sHI", payload.encode('raw_unicode_escape'))
			serial = serial.split("\0".encode('raw_unicode_escape'))[0]
	...
	def sendpacket(self, req, tag, payload={}):
		...
		if isinstance(payload, str):
                        data = struct.pack("IIII", length, self.VERSION, req, tag) + payload.encode('raw_unicode_escape')
		else:                     ### elif isinstance(some_str, str):
	...
	def getpacket(self):
		...
		dlen = struct.unpack("I", dlen.encode('raw_unicode_escape'))[0]
		body = self.socket.recv(dlen - 4)
		version, resp, tag = struct.unpack("III",body.encode('raw_unicode_escape')[:0xc])
