#!/usr/bin/python3
from multiprocessing import Process, Value   # multi-processing support
...
class Motor:
  ...
  def startController(self, qm, qs): ### starts the actual motor controller and processes motor commands
    mytarget = Value('i', 0)         ### starting out with 0 causes the car to "home" on initialization
    mypos = Value('i', 0)            ### position from VESC
    run_process = Process(target=self.start, args=( mytarget, mypos ))
    run_process.start()
    done = False
    while (not done):
      while (not qm.empty()):
        ...
        if m == "quit":
          ...
          mytarget.value = int(num)
          print("target received and sent to motor controller = ", m, mytarget.value, mypos.value)       ###float(num)
      qs.put(mypos.value)
...
#       print("chopped move cnt, rdist vector ", cnt, rdist)
  ...
  """  ----------------------------------------------------------------------------------- """
  """  ----------------------------------------------------------------------------------- """
#  dev home(self, tvsav, forward)
#    cs = Motor.tacho_get(self)
#    forward = False
#    while (cs[3] <= 3.0):  ### look for current buildup while moving
#      Motor.pid-loop(self, destination, direction)
#    cs = Motor.tacho_get(self)
#    tvsave = cs[0]
#    return tvsav
  ...
  def start(self, mytarget, mypos):
    global bus
    done = False
    print( " -*-*-*-*-*-*- initialization motor target value = ", mytarget.value)
    print( " -*-*-*-*-*-*- now we will attempt tp home the car " )
    cs = Motor.tacho_get(self)
    mypos.value = cs[0]
    # Motor.home(self, tvsav, forward)
    while (not done):
        cnt = 0
        if (mytarget.value <= -1 ): return()
        Motor.lock(self)
        mot = MOT.MOT()
...
#       now = datetime.now()
#       print(now.strftime("%H:%M:%S.%f"))
        dd=round((mytarget.value/10.0)*5.755)           # dd now has the number of tachos we need to move
        ...
        rdist = ss[0]
        rvesc = ss[1]
        print( " -*-*-*-*-*-*- start motor target value = ", mytarget.value)
        print("Target = ", mytarget.value, " distance ", dds, " tier ", tier, " direction ", forward)
        print("distance vector ",rdist)
        print("VESC vector is ",rvesc)
        targetsv = mytarget.value
#        done = True
#        target = "t,0"
#        return
        pid = PID.PID(P, I, D)
...
#           break
          ...
          if ( not forward and sp >= -1500 and cnt >= (len(rvesc)/2)): cnt = len(rdist)  # break

### --- move is over so enable pid ( will home in to target position and then hold us there )
        else:
          print("start PID loop here -------------- ",mytarget.value)
          cs = Motor.tacho_get(self)
...
#         scale = min / pid.output         # use first PID output value & create scale control vesc based on min vesc command value
          ...
          print ("PID destination "" PID output = ", pid_out)
          while (mytarget.value == targetsv):
#           break
#           try:
            if (forward):
