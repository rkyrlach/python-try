...
#def msg(nn):
#    switcher={
#       2305:'VESC 2305 ', # bytearray(b'\x00\x00\x00\x00\x00\x00\x00\x00')
#       3585:'VESC 3585 ', # bytearray(b'\x00\x00\x00\x00\x00\x00\x00\x00')
#       3841:'VESC 3841 ', # bytearray(b'\x00\x00\x00\x00\x00\x00\x00\x00')
#       4097:'VESC 4097 ', # bytearray(b'\x01\x1a\xfc\x99\x00\x00\x18\xf5')
#       6913:'VESC 6913 '} # bytearray(b'\x00\x00\x00\x96\x00\xef\x00\x00')
...
def main():
  ...
  while cnt <= 50:
    pos = tacho_get()
    print("pos = ",pos)
    packet = Message(arbitration_id=1, data=[0, 0, 10, 0])
    bus.send(packet)
#   print("data = ", packet, " \n")
    cnt = cnt + 1
