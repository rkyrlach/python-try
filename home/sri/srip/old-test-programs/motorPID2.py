...
def plan(move, tier, min, dir):          # call with the distance you want to move in tachos
    ...
    movesv = 30
    if (move >= 2100): movesv = 140  # greater than 30 foot move subtract about 1 foot
    move = move - movesv
...
#      break
    ...
    if (forward and sp <= 3000 and cnt >= 4): break              # if we are moving slowly, then kick in PID
    if ( not forward and sp >= -3000 and cnt >= 4): break
