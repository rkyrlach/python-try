...
def plan(move, tier, min, dir):          # call with the distance you want to move in tachos
    ...
    if (tier == 1):
      maxI = 5       # set tier 1
      if (move >= 3798): movesv = 0  # >55 feet subtract
      else:
        if (move >= 3453): movesv = 0  # >50 feet subtract
        else:
          if (move >= 3108): movesv = 0  # >45 feet subtract
          else:
            if (move >= 2762): movesv = 0  # >40 feet subtract
            else:
              if (move >= 2417): movesv = 0  # >35 feet subtract
              else:
                if (move >= 2072): movesv = 0  # >30 foot subtract
                else:
                  if (move >= 1726): movesv = 0  # >25 feet subtract
                  else:
                    if (move >= 1380): movesv = 0  # >20 feet subtract
                    else:
                      if (move >= 1034): movesv = 0   # >15 feet subtract
                      else:
                        if (move >= 688): movesv = 0    # >10 feet subtract
                        else:
                          if (move >= 346): movesv = 0    # >5 feet subtract
                          else: movesv = 0
    if (tier == 2):
      maxI = 13      # set tier 2
      if (move >= 3798): movesv = 0  # >55 feet subtract
...
#        print("full move cnt, rvesc vector ", cnt, rvesc)
      ...
      while (not chop and cnt <= cntLimit):           # while (chop and (cnt <= cntsv-2)):
        ...
        cnt += 1
#        print("full move cnt, rdist vector ", cnt, rdist)
#      while (not chop and (cnt >= 1)):         # now do the slowing portion of the vector v[3] and v[5]
#        nxt = erpm_tab(30-cnt, min)
#        rdist.append(nxt[1]+ dist_sv)          # put target "slow" distance into our temp vector
#        if (dir): rvesc.append(nxt[3])         # next vesc positive slow command added to our temporary vector
...
#  print(now.strftime("%H:%M:%S.%f"))
  ...
  rdist = ss[0]
  rvesc = ss[1]
  print("command", command, " distance ", dds, " tier ", tier, " direction ", forward)
